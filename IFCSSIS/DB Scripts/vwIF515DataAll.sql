...
CREATE VIEW [dbo].[vwIF515DataAll]
AS
...
FROM            (SELECT        ID, TYP, TRANSDATA
                          FROM            dbo.IF515_Split) AS a INNER JOIN
                             (SELECT        ID, TYP, TRANSDATA
                               FROM            dbo.IF515_Split AS IF515_Split_2) AS b ON a.ID = b.ID INNER JOIN
                             (SELECT        ID, TYP, TRANSDATA
                               FROM            dbo.IF515_Split AS IF515_Split_1) AS c ON b.ID = c.ID
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwIF515DataAll';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwIF515DataAll';
